...
import { walletCreation } from '../services/walletservice'
...
export const createMQETProducer = async (url: string, queueName: string, exchangeName: string, routekey: string): Promise<any> => {
  const exchangeType: string = 'direct'
  // eslint-disable-next-line no-useless-catch
  try {
    const connection: Connection = await amqp.connect(url)
