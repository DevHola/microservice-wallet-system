...
export const createInitExternalTrans = async (data: externalTransaction): Promise<externalTransaction> => {
  const init = await pool.query('INSERT INTO external_transactions (type,amount,wallet_id,user_id,ref) VALUES ($1,$2,$3,$4,$5) RETURNING id', [data.type, data.amount, data.wallet_id, data.user_id, data.ref])
...
export const getexttransbyid = async (id: string): Promise<externalTransaction> => {
  const find = await pool.query('SELECT type,amount,status,ref FROM external_transactions WHERE id=$1', [id])
